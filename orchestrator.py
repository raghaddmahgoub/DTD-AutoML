...
class DTDPipeline:
    ...
    def stage_automl(self, state: AgentState):
        ...
        print("DEBUG: automl_directives keys:", list(
            state['automl_directives'].keys()))
        print("DEBUG: report content:",
              state['automl_directives'].get('report'))
        print("DEBUG: target_analysis content:",
              state['automl_directives']['report'].get('target_analysis'))
